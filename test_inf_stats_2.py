...
test_run = False
num_trials = 10 #if test_run else 500
num_steps = 1024
desired_block_sizes = [16] #, 8, 16, 32]
...
for desired_block_size in desired_block_sizes:
    ...
    for trial in pbar:
        ...
        inf_budgets_per_step = generation_outputs.inf_budgets
        if len(avg_inf_budgets_per_step) == 0:
            avg_inf_budgets_per_step = inf_budgets_per_step
        else:
            avg_inf_budgets_per_step = [a + b for a, b in zip(avg_inf_budgets_per_step, inf_budgets_per_step)]
    ...
    avg_inf_budgets_per_step = [a / num_trials for a in avg_inf_budgets_per_step]
    # plot bar chart
    import plotly.graph_objects as go
    fig = go.Figure()
    fig.add_bar(
        x=list(range(num_steps)),
        y=avg_inf_budgets_per_step,
        name="Average inf budget per step",
    )
    fig.write_image(f"avg_inf_budgets_per_step_L{L}_desired_block_size_{desired_block_size}.png")
    print(f"Saved figure to avg_inf_budgets_per_step_L{L}_desired_block_size_{desired_block_size}.png")
    exit()
