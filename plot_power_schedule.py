...
for i, v in enumerate(vals):
    ...
    subplot_titles += [
        f"<b>Block diffusion</b> <br> Expected # unmasked = {block_diffusion_expected_active:.1f} <br> Furthest unmasked ahead = {block_diffusion_max_overlap:.1f}",
        f"<b>Staggered noise</b> <br> Expected # unmasked = {staggered_noise_expected_active:.1f} <br> Furthest unmasked ahead = {staggered_noise_max_overlap:.1f}",
    ]
...
for j, int_min in enumerate(int_mins, start=1):
    ...
    if j == 3:
        subplot_titles += [f"Unmasking width: {ns.b:.1f}<br>Expected # unmasked = {expected_active:.1f}<br><b>Furthest unmasked ahead = {max_overlap:.1f}</b>"]
    else:
        subplot_titles += [f"Unmasking width: {ns.b:.1f}<br>Expected # unmasked = {expected_active:.1f}<br>Furthest unmasked ahead = {max_overlap:.1f}"]
...
fig2.update_layout(
    title=f"<b>Staggered noise matched w/ block size {d_fixed}</b>",
    title_x=0.5,
    title_y=0.97,
    template="plotly",
    height=450,
...
fig2.update_yaxes(title_text="Mask prob.")
