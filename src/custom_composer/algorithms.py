...
# Slight tweak on Line 94
# `model_context_manager = model.module.summon_full_params(model.module)` changed to
# `model_context_manager = model.model.summon_full_params(model.module)`
...
def compute_ema(
    ...
    Args:
            model (torch.nn.Module): the model containing the latest weights to use to
                update the moving average weights.
            ema_model (torch.nn.Module, EMAParameters): the model containing the moving
                average weights to be updated.
            smoothing (float, optional): the coefficient representing the degree to
                which older observations are kept.
                ...
                for name, param in itertools.chain(
                    ...
                    if name in ema_params:
                        ema_params[name].copy_(
                            ema_params[name] * smoothing
                            + param.data * (1.0 - smoothing)
                        )
            # Otherwise, the ema model needs to define the named_parameters and
            # named_buffers dictionaries.
            # These should contain the parameters and buffers to average.
                ...
                for name, param in itertools.chain(
                    ...
                    if name in ema_parameters:
                        ema_parameters[name].copy_(
                            ema_parameters[name] * smoothing
                            + param.data * (1.0 - smoothing)
                        )
                    if name in ema_buffers:
                        ema_buffers[name].copy_(
                            ema_buffers[name] * smoothing
                            + param.data * (1.0 - smoothing)
                        )
            else:
...
def get_model_context_manager(model: torch.nn.Module):
    ...
    if fsdp_enabled:
        model_context_manager = model.model.summon_full_params(model.model)  # type: ignore # noqa: E501
    return model_context_manager


class EMA(Algorithm):
    r"""Maintains a set of weights that follow the exponential moving average of the
    ...
    Model evaluation is done with the moving average weights, which can result in better
        generalization. Because of the
    ema weights, EMA can double the model's memory consumption. Note that this does not
        mean that the total memory
    required doubles, since stored activations and the optimizer state are not
        duplicated.
        EMA also uses a small amount of extra compute to update the moving average
        weights.

    See the :doc:`Method Card </method_cards/ema>` for more details.

    Args:
            half_life (str, optional): The time string specifying the half life for
                    terms in the average. A longer half life means old information is
                    remembered longer, a shorter half life means old information is
                    discared sooner. A half life of ``0`` means no averaging is done, an
                    infinite half life means no update is done.
                    Currently only units of epoch ('ep') and batch ('ba'). Time must be
                    an integer value in the units
                    specified. Cannot be used if ``smoothing`` is also specified.
                    Default: ``"1000ba"``.
            smoothing (float, optional): The coefficient representing the degree to
                    which older observations are kept.
                    Must be in the interval :math:`(0, 1)`. Cannot be used if
                    ``half_life`` also specified.
                    This value will not be adjusted if ``update_interval`` is changed.
                    Default: ``None``.
            ema_start (str, optional): The time string denoting the amount of training
                    completed before EMA begins.
                    Currently only units of duration ('dur'), batch ('ba') and epoch
                    ('ep') are supported.
                    Default: ``'0.0dur'``.
            update_interval (str, optional): The time string specifying the period at
                    which updates are done. For example,
                    an ``update_interval='1ep'`` means updates are done every epoch,
                    while ``update_interval='10ba'`` means updates are done once every
                    ten batches.
                    Units must match the units used to specify ``half_life`` if not
                    using ``smoothing``.
                    If not specified, ``update_interval`` will default to ``1`` in the
                    units of ``half_life``, or ``"1ba"`` if ``smoothing`` is specified.
                    Time must be an integer value in the units specified.
                    Default: ``None``.
    ...
    def match(self, event: Event, state: State) -> bool:
        ...
        if (
            ...
            and self.ema_started is False
            and self._should_start(state)
        ):
        ...
        if (
            event in [Event.BATCH_CHECKPOINT, Event.EPOCH_CHECKPOINT]
            and self.ema_started
        ):
            checkpoint_savers = [
                cb for cb in state.callbacks if isinstance(cb, CheckpointSaver)
            ]
        ...
        # Otherwise, always run on events where ema params must be moved after ema start
        ...
        if event == self.update_event and self.ema_started:
            return (
                state.timestamp.get(self.update_interval.unit).value
    ...
    def state_dict(self) -> dict[str, Any]:
        ...
        for attribute_name in self.serialized_attributes:
            if attribute_name == "ema_model":
                ...
                state_dict[attribute_name]["named_parameters_dict"] = (
                    ema_model.named_parameters_dict
                )
                state_dict[attribute_name]["named_buffers_dict"] = (
                    ema_model.named_buffers_dict
                )
            else:
    ...
    def ensure_compatible_state_dict(self, state: dict[str, Any]):
        """Ensure state dicts created prior to Composer 0.13.0 are compatible with later
        versions.
        """
        ...
        # Prior to version 0.13.0, the state dict contained a separate training_model
        # and ema_model.
        # Only one of these needs to be loaded as the ema_model.
        if state["ema_weights_active"] is True:
            # If EMA weights are active, load training weights into ema_model storage
            state_dict = state["training_model"]
        else:
            # If EMA weights are not active, load the ema weights into ema_model storage
        ...
        if isinstance(self.ema_model, EMAParameters):
            ...
            ValueError(
                "ema_model must be initialized before loading state dicts from versions"
                "earlier than 0.13.0"
            )
        ...
        for attribute_name, serialized_value in state_dict.items():
            if attribute_name != "repr":  # skip attribute added by parent class
                if attribute_name == "ema_model":
                    ...
                    self.ema_model.named_parameters_dict = serialized_value[
                        "named_parameters_dict"
                    ]
                    self.ema_model.named_buffers_dict = serialized_value[
                        "named_buffers_dict"
                    ]
                else:
    ...
    def get_ema_model(self, model: torch.nn.Module) -> torch.nn.Module:
        """Replaces the parameters of the supplied model with the ema parameters if they
        ...
        if self.ema_weights_active:
            raise ValueError(
                "The ema weight are currently contained in the composer model."
            )
        self.ema_model.transfer_ema_params(model=model)
        return model

    def get_training_model(self, model: torch.nn.Module) -> torch.nn.Module:
        """Replaces the parameters of the supplied model with the training parameters if
        ...
        """
        assert self.ema_model is not None
        # Ensure that self.ema_model contains the training weights. If not raise error.
        if not self.ema_weights_active:
            raise ValueError(
                "The training weights are currently contained in the composer model."
            )
        self.ema_model.transfer_ema_params(model=model)
        return model
...
class EMAParameters:
    """Class that stores the parameters and buffers of a model needed for averaging."""
        ...
        if model is not None:
            ...
            with model_context_manager:
                ...
                self.named_buffers_dict = {
                    name: buffer.data.clone() for name, buffer in model.named_buffers()
                }
        else:
            # Empty storage
            self.named_parameters_dict = {}
    ...
    def swap_params(self, model: torch.nn.Module):
        ...
        with torch.no_grad():
            ...
            with model_context_manager:
                for name, param in model.named_parameters():
                    if name in ema_params:
                        # Use copy vs. raw data access (eg .data) doesn't work with FSDP
    ...
    def transfer_ema_params(self, model: torch.nn.Module):
        """Transfers the parameters and buffers from the ema model to the supplied
        model.
        """
        model_context_manager = get_model_context_manager(model)
    ...
    def move_params_to_device(self, destination_model: torch.nn.Module):
        ...
        with model_context_manager:
            for name, param in destination_model.named_parameters():
                if name in self.named_parameters_dict:
                    self.named_parameters_dict[name] = self.named_parameters_dict[
                        name
                    ].to(param.device)

            for name, buffer in destination_model.named_buffers():
                if name in self.named_buffers_dict:
                    self.named_buffers_dict[name] = self.named_buffers_dict[name].to(
                        buffer.device
                    )
