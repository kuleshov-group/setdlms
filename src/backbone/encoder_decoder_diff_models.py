...
class EncToDecMLP(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.gate_proj = nn.Linear(encoder_hidden_size, intermediate_size, bias=False)
        self.up_proj = nn.Linear(encoder_hidden_size, intermediate_size, bias=False)
        self.down_proj = nn.Linear(intermediate_size, decoder_hidden_size, bias=False)
...
class LLMasEncoderDecoderDiffModels(nn.Module):
    def __init__(
        self,
        ...
        **llm_init_kwargs,
        ...
        self.enc_to_dec_network = EncToDecMLP(
            encoder_hidden_size=self.encoder.config.hidden_size,
            decoder_hidden_size=self.decoder.config.hidden_size,
            intermediate_size=self.decoder.config.intermediate_size,
            hidden_act=self.decoder.config.hidden_act,
        )
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        if encoder_last_hidden_state is None:  # No new encoder tokens
            ...
            encoder_last_hidden_state = self.enc_to_dec_network(
