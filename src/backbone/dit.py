...
def apply_rotary_pos_emb_to_tensor(x, cos, sin):
...
class DDiTBlock(nn.Module):
    ...
    def _attention_with_kv_cache(
        ...
        with torch.amp.autocast("cuda", enabled=False):
            cos, sin = rotary_cos_sin
            cos = cos.to(qkv.dtype)[:, :, 0]
            sin = sin.to(qkv.dtype)[:, :, 0]
            q = apply_rotary_pos_emb_to_tensor(q, cos, sin)
            k = apply_rotary_pos_emb_to_tensor(k, cos, sin)
        k, v = self._process_and_update_kv(k=k, v=v, num_clean=num_clean)
        ...
        attention_bias = self.neg_infinity * torch.triu(ones, diagonal=1)
        attention_bias = attention_bias.view(
            1, 1, num_clean_and_mask, num_clean_and_mask
        )
        attn_scores[:, :, :, -num_clean_and_mask:] += attention_bias
...
class DIT(nn.Module, huggingface_hub.PyTorchModelHubMixin):
        ...
        if pretrained_model_name_or_path is not None:
            if os.path.exists(pretrained_model_name_or_path):
                state_dict = torch.load(
                    pretrained_model_name_or_path, weights_only=False
                )
    ...
    def forward_sample(
        ...
        zeros = torch.zeros(
            zt.shape[0], device=zt.device, dtype=self.vocab_embed.embedding.dtype
        )
        t_cond = F.silu(self.sigma_map(zeros)) if self.adaLN else None
        for i in range(len(self.blocks)):
            x = self.blocks[i](
                x,
                rotary_cos_sin,
